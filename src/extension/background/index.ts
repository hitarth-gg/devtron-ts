...
chrome.runtime.onConnect.addListener((port) => {
  if (port.name === PORT_NAME.PANEL) {
    ...
    port.onMessage.addListener((message: MessagePanel) => handlePanelMessage(message));
    ...
    port.onMessage.addListener((message: MessageContentScript) => handleContentMessage(message));
    port.onDisconnect.addListener(() => {
...
function logSomeInfo(msg: any): void {
  console.log(msg);
  console.log('LOG FROM TWN WORLD.');
}
(globalThis as any).logSomeInfo = logSomeInfo;
